...
import {
  ...
  Users, Plus, Trash2, Pencil, Check, Sun, Moon, Monitor,
  ChevronRight, ToggleLeft, Volume2, Zap, Quote,
} from 'lucide-react';
    ...
    >
        ...
        <Section title={t('appearance')}>
          ...
          <Row icon={Globe} label={t('language')}>
            <SegmentedPicker
              ...
              onChange={v => {
                onUpdate({ language: v as Language });
          ...
          <Row icon={ToggleLeft} label={t('animations')} desc={t('animationsDesc')}>
            <Switch checked={settings.animationsEnabled}
              onCheckedChange={v => onUpdate({ animationsEnabled: v })} />
          </Row>
          <Row icon={Quote} label={t('showMotivation')} divider={false}>
            <Switch checked={settings.showMotivation}
      ...
      <motion.div variants={itemV}>
        <Section title={t('hydration')}>
          ...
          <Row icon={Zap} label={t('hydrationFactors')} desc={t('hydrationFactorsDesc')}>
            <Switch checked={settings.useHydrationFactors}
              onCheckedChange={v => onUpdate({ useHydrationFactors: v })} />
          </Row>
          <Row icon={Droplets} label={t('showHydrationFactor')} desc={t('showHydrationFactor')} divider={false}>
            <Switch
      ...
      <motion.div variants={itemV}>
        <Section title={t('notifications')}>
          <Row icon={Bell} label={t('goalAchieved')}>
            <Switch checked={settings.notifications.goalAchieved}
              onCheckedChange={v => onUpdate({
                notifications: { ...settings.notifications, goalAchieved: v }
              })} />
          </Row>
          <Row icon={Volume2} label={t('streakReminder')}>
            <Switch checked={settings.notifications.streakReminder}
              onCheckedChange={v => onUpdate({
                notifications: { ...settings.notifications, streakReminder: v }
              })} />
          </Row>
          <Row icon={Sun} label={t('morningMotivation')} desc={`at ${settings.notifications.morningHour}:00`} divider={false}>
            <div className="flex items-center gap-2">
              <select
                value={settings.notifications.morningHour}
                onChange={e => onUpdate({
                  notifications: { ...settings.notifications, morningHour: Number(e.target.value) }
                })}
                disabled={!settings.notifications.morningMotivation}
              ...
              <Switch checked={settings.notifications.morningMotivation}
                onCheckedChange={v => onUpdate({
                  notifications: { ...settings.notifications, morningMotivation: v }
                })} />
            </div>
          </Row>
        </Section>
          ...
          <CardHeader className="pb-0 pt-4 px-4 flex-row items-center justify-between">
            ...
            {profiles.length < 8 && (
              <Button size="sm" variant="ghost" onClick={openAddProfile}
                className="h-7 px-2 gap-1 text-sky-500 hover:text-sky-600 hover:bg-sky-500/10">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the profile and all its drink logs.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="bg-destructive text-destructive-foreground"
              onClick={() => {
                if (showDeleteProfile) deleteProfile(showDeleteProfile);
      ...
      <AlertDialog open={showResetConfirm} onOpenChange={setShowResetConfirm}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>{t('cancel')}</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground"
              onClick={() => { onResetData(); setShowResetConfirm(false); onToast(t('dataReset')); }}>
              {t('resetConfirm')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
